...
static void enqueue_task_in_userspace(struct task_struct *p) {
	...
	if (bpf_map_push_elem(&enqueued, &task, 0) != 0) {
		...
		if (time_before(vtime, global_vtime - SCX_SLICE_DFL))
...
static void dequeue_tasks_from_userspace(void) {
    ...
    bpf_repeat(MAX_ENQUEUED_TASKS) {
        ...
        if (!tctx) {
            ...
            // Don't have a task context, use default slice, also don't create one.
			...
			tctx->has_slice = true; // Mark that this task now has an assigned slice from userspace
        ...
        // SATURATION FALLBACK & IDLE WAKEUP (applied to both branches)
...
int create_task_ctx(struct task_struct *p) {
	...
	struct task_ctx tctx_init = {
		...
		.has_slice = false, // Initialize as false
	};
...
s32 BPF_STRUCT_OPS(serverless_select_cpu, struct task_struct *p, s32 prev_cpu, u64 wake_flags) {
	...
	if (is_idle) {
		...
		// Prevent direct dispatch if we are still waiting for a slice from userspace.
		// This forces the task to fall through to serverless_enqueue() where it will be paused.
		if (!tctx->has_slice) {
			return cpu;
		}
...
s32 BPF_STRUCT_OPS(serverless_enqueue, struct task_struct *p, u64 enq_flags) {
	...
	// BLOCK THE TASK: If userspace hasn't assigned a slice, do not insert into any DSQ.
	if (!tctx->has_slice) {
		DEBUG_PRINTK("%-30s task %d waiting for userspace slice", "[serverless_enqueue]", p->pid);
		return 0;
	}
...
void BPF_STRUCT_OPS(serverless_running, struct task_struct *p) {
	...
	// live with it.
	bool cond = time_before(global_vtime, p->scx.dsq_vtime);

	if (cond) {
		global_vtime = p->scx.dsq_vtime;
	}

	DEBUG_PRINTK("%-30s task %d running, ran_time %llu, rem_slice %llu, new global global_vtime %llu, cond=%d", "[serverless_running]", p->pid, p->scx.dsq_vtime, p->scx.slice, global_vtime, cond);
